# web/stdlib.py - DhiCode Extended Standard Library for WebAssembly
import math
import time
import os
import sys
...
def get_stdlib_modules(obj_factory) -> Dict[str, Dict[str, Any]]:
    ...
    def nakaiy_lookup(*args):
        if len(args) < 2:
            return obj_factory['error']("ނަކަތް_ހޯދާ() އަށް މަހާއި ދުވަސް ދޭންވާނެ")
    ...
    # =========================================================================
    ONES_DHIVEHI = ["", "އެއް", "ދެ", "ތިން", "ހަތަރު", "ފަސް", "ހަ", "ހަތް", "އަށް", "ނުވަ"]
    ONES_STANDALONE = ["ސުމެއް", "އެކެއް", "ދޭއް", "ތިނެއް", "ހަތަރެއް", "ފަހެއް", "ހައެއް", "ހަތެއް", "އަށެއް", "ނުވައެއް"]
    TEENS = ["ދިހައެއް", "އެގާރަ", "ބާރަ", "ތޭރަ", "ސާދަ", "ފަނަރަ", "ސޯޅަ", "ސަތާރަ", "އަށާރަ", "ނަވާރަ"]
    TENS = ["", "ދިހަ", "ވިހި", "ތިރީސް", "ސާޅީސް", "ފަންސާސް", "ފަސްދޮޅަސް", "ހައްދިހަ", "އައްޑިހަ", "ނުވަދިހަ"]
    TWENTIES = ["ވިހި", "އެކާވީސް", "ބާވީސް", "ތޭވީސް", "ސައުވީސް", "ފަންސަވީސް", "ސައްބީސް", "ސަތާވީސް", "އަށާވީސް", "ނަވާވީސް"]

    THAANA_ALPHABET = ['ހ', 'ށ', 'ނ', 'ރ', 'ބ', 'ޅ', 'ކ', 'އ', 'ވ', 'މ', 'ފ', 'ދ', 'ތ', 'ލ', 'ގ', 'ޏ', 'ސ', 'ޑ', 'ޒ', 'ޓ', 'ޔ', 'ޕ', 'ޖ', 'ޗ']
